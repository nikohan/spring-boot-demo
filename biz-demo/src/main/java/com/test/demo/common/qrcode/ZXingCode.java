package com.test.demo.common.qrcode;
...
/**
 * 来自：http://blog.csdn.net/u013142781/article/details/50310419
 *
 * @description 二维码
 * @author luoguohui
...
//      String filePath = (javax.servlet.http.HttpServletRequest)request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/llhlogo.png";
		...
		String filePath = "D:/Data/test.jpg";  //TODO
		String content = qrUrl;
		try {
			ZXingCode zp = new ZXingCode();
			BufferedImage bim = zp.getQRCodeBufferedImage(
					content, BarcodeFormat.QR_CODE, 400, 400, zp.getDecodeHintType());
			return zp.addLogoQRCode(bim, new File(filePath), new LogoConfig(), productName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 * @param logoPic
	 */
	public String addLogoQRCode(BufferedImage bim,
								File logoPic,
		...
		try {
			...
			int widthLogo = logo.getWidth(null) > image.getWidth() * 3 / 10 ?
					(image.getWidth() * 3 / 10) : logo.getWidth(null);
			int heightLogo = logo.getHeight(null) > image.getHeight() * 3 / 10 ?
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
			...
			ImageIO.write(image, "png", new File("D:/Data/TDC-" + new Date().getTime() + "test.png")); //TODO
